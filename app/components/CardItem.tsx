  ...
  return (
    <>
      ...
      >
        ...
        {description && <p className="text-sm text-gray-600 truncate max-w-[200px]">{description}</p>}
      </div>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
          <div className="bg-white p-6 rounded shadow-lg w-2/3 max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <input value={newTitle} onChange={(e) => setNewTitle(e.target.value)} className="w-full p-3 border rounded mb-4 outline-none text-lg" placeholder="Title" />
            <textarea value={newDesc} onChange={(e) => setNewDesc(e.target.value)} className="w-full p-3 border rounded mb-4 outline-none text-md resize-none min-h-[150px]" placeholder="Description" />
            <div className="flex justify-end gap-3 mt-2">
              ...
              <button onClick={() => { onDelete(_id); setModalOpen(false); }}
